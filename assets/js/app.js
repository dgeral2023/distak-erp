...
async function refresh(){await refreshData();renderClientes();renderObras();renderOrcamentos();renderCustos();renderPagamentos();renderFuncionarios();renderDashboard();renderAgenda();renderV3();renderHybridMenu();renderAssistantInsight();if(store.profile?.role!=="admin")$("funcionarioObras").innerHTML=$("obrasTable").innerHTML}
...
initCustos(refresh);initOrcamentos();initFuncionarios(refresh);initFotografias();initDocumentos();initDiario();initAgenda(refresh);initV3();initHybridMenu({openCliente,openObra,openOrcamento,openCusto,openPagamento,openAgendaTask});initAssistant();initPwa();
