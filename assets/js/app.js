...
async function refresh(){await refreshData();renderClientes();renderObras();renderOrcamentos();renderCustos();renderPagamentos();renderFuncionarios();renderDashboard();renderAgenda();renderPrevisoes();renderDossies();renderOperacional();renderCompras();renderMedicoes();renderV3();renderHybridMenu();renderAssistantInsight();if(store.profile?.role!=="admin")$("funcionarioObras").innerHTML=$("obrasTable").innerHTML}
...
initCustos(refresh);initOrcamentos();initFuncionarios(refresh);initFotografias();initDocumentos();initDiario();initAgenda(refresh);initPrevisoes(refresh);initDossies();initOperacional(refresh);initCompras(refresh);initMedicoes(refresh);initV3();initHybridMenu({openCliente,openObra,openOrcamento,openCusto,openPagamento,openAgendaTask,openForecast,openOperationalQuick});initAssistant();initPwa();
