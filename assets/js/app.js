...
async function refresh(){await refreshData();renderClientes();renderObras();renderOrcamentos();renderCustos();renderPagamentos();renderFuncionarios();renderDashboard();renderV3();renderHybridMenu();if(store.profile?.role!=="admin")$("funcionarioObras").innerHTML=$("obrasTable").innerHTML}
...
initCustos(refresh);initOrcamentos();initFuncionarios(refresh);initFotografias();initDocumentos();initDiario();initV3();initHybridMenu({openCliente,openObra,openOrcamento,openCusto,openPagamento});initAssistant();
