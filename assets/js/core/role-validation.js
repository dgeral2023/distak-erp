...
export function buildRoleValidationPlan({profiles=[],assignments=[],clientAccess=[],works=[],clients=[],portalWorks=[]}={}){
  ...
  const roles=[
    ...
    {role:"team",label:"Equipa",ready:Boolean(team),accountId:team?.id||null,reasons:team?[]:[active.some(row=>teamRoles.has(row.role))?"A conta de Equipa ativa ainda não tem uma obra válida atribuída.":"Falta uma conta de Equipa ativa."]},
  ...
  return {format:"distak-role-validation-plan",version:1,generatedAt:new Date().toISOString(),mode:"preparation-only",realValidationPerformed:false,ready:roles.every(row=>row.ready),roles:roles.map(row=>({...row,scenarios:validationScenarios.filter(item=>item.role===row.role)})),guardrails:["Não criar ou convidar contas sem autorização específica.","Não usar dados financeiros ou operacionais fora do ERP.","Não considerar os perfis validados sem execução real separada no computador e telemóvel."]};
}
