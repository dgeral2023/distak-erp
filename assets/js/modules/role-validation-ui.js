...
function openPlan(){
  const plan=currentPlan(),scoped=plan.roles.filter(row=>row.inScope),ready=scoped.filter(row=>row.ready).length,scenarios=scoped.reduce((sum,row)=>sum+row.scenarios.length,0);
  $("roleValidationSummary").innerHTML=`<article class="${plan.ready?"ready":""}"><span>Perfis preparados</span><strong>${ready}/${scoped.length}</strong><small>${plan.ready?"Pré-condições completas":"Ainda existem pendências"}</small></article><article><span>Cenários desta fase</span><strong>${scenarios}</strong><small>Computador e telemóvel</small></article><article><span>Validação real</span><strong>Pendente</strong><small>Executar com cada conta</small></article>`;
  $("roleValidationRoles").innerHTML=plan.roles.map(row=>`<section class="validation-role ${row.ready&&row.inScope?"ready":""}"><header><div><strong>${esc(row.label)}</strong><small>${row.inScope?`${row.scenarios.length} cenário(s) preparado(s)`:"Fora do escopo desta fase"}</small></div><span>${row.inScope?(row.ready?"Pré-condições prontas":"Preparação pendente"):"Adiado"}</span></header>${row.inScope&&row.reasons.length?`<p>${esc(row.reasons.join(" "))}</p>`:""}${row.inScope?`<div class="validation-scenarios">${row.scenarios.map(scenario=>`<article><small>${esc(scenario.device)}</small><strong>${esc(scenario.title)}</strong><ul>${scenario.checks.map(check=>`<li>${esc(check)}</li>`).join("")}</ul></article>`).join("")}</div>`:""}</section>`).join("");
