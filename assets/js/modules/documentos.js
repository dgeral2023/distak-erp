...
async function deleteDoc(id){const d=documentos.find(x=>String(x.id)===String(id));if(!d||!confirm(`Eliminar "${d.nome}"?`))return;try{if(d.custo_id){const {error}=await db.from("custos").update({anexo_path:null,anexo_nome:null,anexo_mime_type:null,anexo_tamanho_bytes:null}).eq("id",d.custo_id);if(error)throw error;const {error:s}=await db.storage.from(BUCKET).remove([d.storage_path]);if(s)throw s;toast("Fatura removida do custo e dos documentos.")}else{const {error:s}=await db.storage.from(BUCKET).remove([d.storage_path]);if(s)throw s;const {error}=await db.from("obra_documentos").delete().eq("id",d.id);if(error)throw error;toast("Documento eliminado.")}await renderObraDocumentos(obraAtual)}catch(e){toast(e.message,"error")}}
