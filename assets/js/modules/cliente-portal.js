...
function openInvite(){
  $("clientInviteForm").reset();$("clientInviteClient").innerHTML=`<option value="">Selecionar cliente</option>${(store.clientes||[]).map(row=>`<option value="${row.id}">${esc(row.nome||`Cliente ${row.id}`)}</option>`).join("")}`;$("clientInviteDialog").showModal();
}

export function initClientePortal(refresh){
  $("clientPublishNew")?.addEventListener("click",()=>openPublication());$("clientApprovalNew")?.addEventListener("click",openApproval);$("clientInviteNew")?.addEventListener("click",openInvite);
  ...
  $("clientInviteForm")?.addEventListener("submit",async event=>{event.preventDefault();const button=event.currentTarget.querySelector('button[type="submit"]');button.disabled=true;try{const {data,error}=await db.functions.invoke("convidar-cliente",{body:{cliente_id:Number($("clientInviteClient").value),nome:$("clientInviteName").value.trim(),email:$("clientInviteEmail").value.trim().toLowerCase()}});if(error)throw error;if(data?.error)throw new Error(data.error);$("clientInviteDialog").close();await refresh();toast("Convite enviado e acesso do cliente associado.")}catch(error){toast(error.message||"Não foi possível enviar o convite.","error")}finally{button.disabled=false}});
  document.addEventListener("click",async event=>{const edit=event.target.closest("[data-client-publish-edit]")?.dataset.clientPublishEdit;if(edit)openPublication(store.clientePortalObras.find(row=>String(row.id)===edit));const button=event.target.closest("[data-client-approval]");if(button){const decision=button.dataset.decision;if(!confirm(decision==="aprovado"?"Confirmar esta aprovação?":"Pedir revisão deste item?"))return;button.disabled=true;try{const {error}=await db.rpc("responder_cliente_portal_aprovacao",{p_aprovacao_id:button.dataset.clientApproval,p_decisao:decision});if(error)throw error;await refresh();toast(decision==="aprovado"?"Aprovação registada.":"Pedido de revisão registado.")}catch(error){toast(error.message||"Não foi possível registar a resposta.","error")}finally{button.disabled=false}}});
}
