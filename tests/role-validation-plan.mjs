...
assert.equal(incomplete.ready,false);assert.equal(incomplete.realValidationPerformed,false);assert.equal(incomplete.mode,"preparation-only");assert.deepEqual(incomplete.releaseScope,["admin","team"]);assert.equal(incomplete.roles.find(row=>row.role==="admin").ready,true);assert.equal(incomplete.roles.find(row=>row.role==="team").ready,false);assert.equal(incomplete.roles.find(row=>row.role==="client").status,"deferred");
...
assert.equal(complete.ready,true);assert.equal(complete.roles.every(row=>row.scenarios.length===2),true);assert.equal(complete.roles.find(row=>row.role==="client").inScope,false);assert(complete.guardrails.some(row=>row.includes("autorização específica")));assert(complete.guardrails.some(row=>row.includes("Cliente está adiado")));assert.equal(JSON.stringify(complete).includes("pagamentos"),false);
...
console.log("Plano aprovado: Administrador e Funcionário no escopo atual; Cliente adiado sem bloquear a publicação.");
