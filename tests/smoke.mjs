...
const requiredIds = [
  ...
  ,"option5ActiveWorks", "option5ExecutionValue", "option5Income", "option5Alerts", "statValorSemIva", "statValorIva"
  ,"obraMorada", "obraMoradaSuggestions", "obraMoradaHelp", "obraIvaTaxa", "obraIvaValorPreview", "obraTotalPreview", "obraFichaBase", "obraFichaIvaLabel", "obraFichaIva", "dashboardVatChart"
  ...
  ,"view-subempreiteiros", "newSupplierBtn", "newSubcontractBtn", "subcontractKpis", "subcontractSearch",
  "subcontractWorkFilter", "subcontractStateFilter", "subcontractList", "subcontractSuppliers",
  "supplierDialog", "supplierForm", "subcontractDialog", "subcontractForm", "subcontractChangeDialog",
  "subcontractChangeForm", "custoSubempreitadaId", "obra-tab-subempreitadas", "dashboardSubcontractSummary"
  ,"subcontractVatMode", "subcontractVatSingleFields", "subcontractVatPartsSection", "subcontractVatParts",
  "subcontractVatSummary", "subcontractVatValuePreview", "subcontractTotalPreview"
];
...
for (const required of ["initHybridMenu", "renderHybridMenu", "distakSidebarCollapsed", "event.ctrlKey", "distakFavoriteViews", "distakMenuGroups", "Pedidos do site", "portal-admin"]){
...
for (const required of ["distak-shell-v3.8-subcontract-mixed-vat-20260824-v1", "request.mode==='navigate'", "url.origin!==self.location.origin", "ignoreSearch:true", "async response", "await cache.put", "SHELL.map(async path", "fetch(path,{cache:'no-store'})", "['style','script','manifest'].includes", "fetch(request,{cache:'no-store'}).then(updateCache).catch", "assets/css/accessibility.css", "assets/css/client-access-management.css", "assets/css/cliente-approvals.css", "assets/css/backup.css", "assets/css/password-recovery.css", "assets/css/portal-content-management.css", "assets/css/role-validation.css", "assets/css/subempreiteiros.css", "assets/css/user-management.css", "assets/js/config.js", "assets/js/app.js", "assets/js/core/access-management.js", "assets/js/core/accessibility.js", "assets/js/core/assistant-local.js", "assets/js/core/backup-readiness.js", "assets/js/core/bootstrap-errors.js", "assets/js/core/client-addresses.js", "assets/js/core/data-quality.js", "assets/js/core/dossier-quality.js", "assets/js/core/field-queue.js", "assets/js/core/intelligence-actions.js", "assets/js/core/portal-publication.js", "assets/js/core/role-validation.js", "assets/js/core/subcontract-finance.js", "assets/js/core/work-finance.js", "assets/js/core/work-vat-parts.js", "assets/js/core/workload-analysis.js", "assets/js/modules/backup.js", "assets/js/modules/campo.js", "assets/js/modules/inteligencia.js", "assets/js/modules/obra-iva-misto.js", "assets/js/modules/portal-content-admin.js", "assets/js/modules/cliente-portal.js", "assets/js/modules/subempreiteiros.js"]){
...
for (const required of ["renderMedicoes", "initMedicoes", "saveMeasurementWithItems", "quantidade_anterior", "data-remove-measurement-item", "Nenhum pagamento foi criado"]){
...
for (const required of ["renderClientePortal", "renderClientePortalAdmin", "initClientePortal", "clientePortalObras", "clientePortalAtualizacoes", "clientePortalFicheiros", "clientePortalAprovacoes", "Progresso comunicado", "data-client-publish-edit", "data-client-approval", "responder_cliente_portal_aprovacao", "Nenhuma alteração automática", "safeHttpsUrl"]){
  check(clientPortalModule.includes(required), `Portal do cliente incompleto: ${required}`);
}
const portalSafety=readFileSync(join(root,"assets","js","core","portal-publication.js"),"utf8");
for(const required of ['url.protocol==="https:"','!url.username','!url.password'])check(portalSafety.includes(required),`Validação de URL do portal incompleta: ${required}`);
