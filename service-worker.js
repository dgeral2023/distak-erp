const CACHE='distak-shell-v3.8-functional-audit-20260820-v1';
const SHELL=[
...
self.addEventListener('fetch',event=>{
  ...
  if(['style','script','manifest'].includes(request.destination)){
    event.respondWith(fetch(request).then(updateCache).catch(()=>caches.match(request,{ignoreSearch:true})));
    return;
  }
